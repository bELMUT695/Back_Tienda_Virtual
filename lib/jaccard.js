...
function typeCheckCoOccurrenceMatrix(coMatrix, numItems) {
  if (!(coMatrix instanceof math.Matrix)) {
    throw new TypeError(
      "The occurrence matrix should be a mathJS Matrix object generated by createCoMatrix"
    );
  }
...
function createCoMatrix(ratings) {
  ...
  for (let y = 0; y < nUsers; y += 1) {
    ...
    for (let x = 0; x < nItems - 1; x += 1) {
      ...
      for (let index = x + 1; index < nItems; index += 1) {
        // Co-occurrence
        if (
          allowedRatings.includes(ratings[y][x]) &&
          allowedRatings.includes(ratings[y][index])
        ) {
  ...
  const JaccardBest = math.dotDivide(
    1,
    math.add(
      1,
      math.add(
        math.add(
          math.dotDivide(1, coMatrixCalif),
          math.dotDivide(coMatriA, math.add(1, coMatriA))
        ),
        math.dotDivide(1, math.add(1, normalizerMatrix))
      )
    )
  );
...
function collaborativeFilterBasedItems(ratings) {
  if (!Array.isArray(ratings)) return false;
  const coMatrixDenominador = createCoMatrix(ratings);

  return coMatrixDenominador;
}

function collaborativeFilterBasedItemsRecommendations(
  ratings,
  coRatedUserItems,
  coMatrixDenominador
) {
  if (!Array.isArray(ratings)) return false;
  const recommendations = getRecommendationsJaccard(
    ratings,
    coMatrixDenominador,
    coRatedUserItems
  );
...
module.exports = {
  ...
  CFilterJaccardRecomendation: collaborativeFilterBasedItemsRecommendations,
};
